# Command: setup:integrations

**Purpose:** Interactive setup for Jira, Confluence, Slack, and Figma integrations

**Category:** Setup

**Usage:**
```
setup:integrations
```

---

## 📋 Execution

When you run this command, Claude will execute:
```bash
bash .claude/scripts/setup-integrations.sh
```

This script will:
1. ✅ Check prerequisites (shell, direnv, existing config)
2. ✅ Install direnv (if not installed) - prompts for permission
3. ✅ **Interactive prompts** - asks for each integration's credentials
4. ✅ Generate `.claude/.envrc` with your inputs
5. ✅ Update `.claude/.gitignore` to ignore secrets
6. ✅ Create/update project `.envrc` to load CCPM config
7. ✅ Allow direnv and reload variables
8. ✅ Test all configured integrations

**IMPORTANT:** 
- All secrets stay in `.claude/.envrc` (git-ignored)
- Main project gets clean `.envrc` (no secrets, can commit)
- Fully interactive - you input values when prompted

---

## 🔄 Workflow

### Phase 1: Check Prerequisites

**Steps:**
1. Check if direnv is installed
2. Check shell configuration
3. Check if `.claude/.envrc` exists

**Output:**
```markdown
## Prerequisites Check

✅ Shell: zsh detected
❌ direnv: Not installed
ℹ️  .envrc: Not found

Next: Install direnv
```

---

### Phase 2: Install direnv (Optional)

**If direnv not installed:**

```markdown
## Install direnv

direnv allows automatic loading of environment variables
when you enter the project directory.

Do you want to install direnv?
Options:
- "yes" → Install direnv automatically
- "skip" → Skip installation (manual setup required)
```

**If user approves:**
- Detects OS (macOS, Linux, Windows)
- Runs appropriate install command
- Adds hook to shell config
- Verifies installation

**Auto-runs:**
```bash
# macOS
brew install direnv

# Linux (Ubuntu/Debian)
sudo apt-get install direnv

# Linux (Arch)
sudo pacman -S direnv
```

**Adds to shell config:**
```bash
# For zsh
echo 'eval "$(direnv hook zsh)"' >> ~/.zshrc

# For bash
echo 'eval "$(direnv hook bash)"' >> ~/.bashrc
```

---

### Phase 3: Create `.claude/.envrc`

**Interactive Configuration:**

```markdown
## Configure Integrations

I'll guide you through setting up each integration.
You can skip any integration by typing "skip".

Press Enter to continue...
```

#### 3.1: Jira Configuration

```markdown
### Jira Integration

Jira URL (e.g., https://company.atlassian.net):
> [user input]

Jira Email:
> [user input]

Jira API Token (get from: https://id.atlassian.com/manage-profile/security/api-tokens):
> [user input - hidden]

Jira Project Key (e.g., PROJ, PROJ):
> [user input]

✅ Jira configured!
```

#### 3.2: Confluence Configuration

```markdown
### Confluence Integration

Use same credentials as Jira? (yes/no):
> yes

Confluence Space Key (e.g., TECH):
> [user input]

✅ Confluence configured!
```

#### 3.3: Slack Configuration

```markdown
### Slack Integration

Choose setup method:
1. Bot Token (recommended - more features)
2. Webhook URL (simpler - notifications only)
3. Skip

> [user input: 1, 2, or 3]

[If 1 - Bot Token]
Slack Bot Token (get from: https://api.slack.com/apps):
> [user input - hidden]

Slack Channel ID:
> [user input]

[If 2 - Webhook]
Slack Webhook URL:
> [user input - hidden]

✅ Slack configured!
```

#### 3.4: Figma Configuration

```markdown
### Figma Integration

Figma Personal Access Token (get from: Figma → Settings → Personal access tokens):
> [user input - hidden]

Default Figma File Key (optional, press Enter to skip):
> [user input or Enter]

✅ Figma configured!
```

---

### Phase 4: Generate `.claude/.envrc`

**Creates file with user input:**

```bash
# .claude/.envrc - CCPM Integration Configuration
# Auto-generated by setup:integrations command
# Date: 2025-11-25

# ============================================
# Jira Integration
# ============================================
export JIRA_URL="https://company.atlassian.net"
export JIRA_EMAIL="user@company.com"
export JIRA_API_TOKEN="***"
export JIRA_PROJECT_KEY="PROJ"
export JIRA_DEFAULT_PRIORITY="Medium"

# ============================================
# Confluence Integration
# ============================================
export CONFLUENCE_URL="https://company.atlassian.net/wiki"
export CONFLUENCE_EMAIL="$JIRA_EMAIL"
export CONFLUENCE_API_TOKEN="$JIRA_API_TOKEN"
export CONFLUENCE_SPACE_KEY="TECH"
export CONFLUENCE_DEFAULT_LABELS="ccpm,generated,documentation"

# ============================================
# Slack Integration
# ============================================
export SLACK_BOT_TOKEN="***"
export SLACK_CHANNEL_ID="C1234567890"

# ============================================
# Figma Integration
# ============================================
export FIGMA_ACCESS_TOKEN="***"
export FIGMA_MCP_ENABLED="true"

# ============================================
# CCPM Configuration
# ============================================
export CCPM_AUTO_APPROVE="true"
export CCPM_DEFAULT_COVERAGE="80"
export CCPM_TDD_ENFORCE="true"
export CCPM_AUTO_NOTIFY="true"
```

**Updates `.gitignore`:**
```bash
# Add to .claude/.gitignore
.envrc
.envrc.local
*.env
```

---

### Phase 5: Setup direnv for `.claude/.envrc`

**Create `.claude/.envrc` loader:**

The trick is to use direnv's `source_env` feature to load from `.claude/`.

**In project root, create `.envrc` (if doesn't exist):**
```bash
# .envrc - Load CCPM integrations
# This file can be committed (no secrets)

# Load CCPM integration config
if [ -f .claude/.envrc ]; then
  source_env .claude/.envrc
fi

# Your other project env vars can go here
```

**Or append to existing `.envrc`:**
```bash
# Append to existing .envrc
echo "" >> .envrc
echo "# CCPM Integration" >> .envrc
echo "if [ -f .claude/.envrc ]; then" >> .envrc
echo "  source_env .claude/.envrc" >> .envrc
echo "fi" >> .envrc
```

---

### Phase 6: Allow direnv

**Auto-run:**
```bash
# Allow direnv to load .envrc
direnv allow .

# Reload shell
source ~/.zshrc  # or ~/.bashrc
```

**Output:**
```markdown
## direnv Configuration Complete

✅ .claude/.envrc created
✅ Project .envrc updated to load CCPM config
✅ direnv allowed

Environment variables will auto-load when you:
- cd into this project
- Open new terminal in this project
```

---

### Phase 7: Test Integrations

**Auto-run test script:**
```bash
./.claude/scripts/test-integrations.sh
```

**Output:**
```markdown
## Testing Integrations

🧪 CCPM Integration Tests
==========================

Testing Jira connection... ✅ PASSED
Testing Confluence connection... ✅ PASSED
Testing Slack webhook... ✅ PASSED
Testing Figma connection... ✅ PASSED

==========================
Test Summary
==========================
Passed: 4
Failed: 0

✅ All configured integrations working!
```

---

### Phase 8: Update Auto-Approval Settings

**Adds to `.claude/settings.local.json`:**
```json
{
  "permissions": {
    "allow": [
      "# direnv commands",
      "Bash(brew install direnv:*)",
      "Bash(sudo apt-get install direnv:*)",
      "Bash(direnv allow:*)",
      "Bash(direnv reload:*)",
      "Bash(source ~/.zshrc:*)",
      "Bash(source ~/.bashrc:*)",
      
      "# Integration test",
      "Bash(./.claude/scripts/test-integrations.sh:*)"
    ]
  }
}
```

---

## 📁 File Structure After Setup

```
project/
├── .envrc                    # Loads .claude/.envrc (can commit)
│   └── "source_env .claude/.envrc"
│
└── .claude/
    ├── .envrc                # CCPM integrations (git-ignored)
    │   └── Contains: JIRA_*, CONFLUENCE_*, SLACK_*, FIGMA_*
    │
    ├── .gitignore            # Updated to ignore .envrc
    │   └── Added: .envrc, *.env
    │
    └── settings.local.json   # Updated with direnv permissions
```

**Key Points:**
- ✅ Main project folder: Only `.envrc` (no secrets, can commit)
- ✅ All secrets in: `.claude/.envrc` (git-ignored)
- ✅ Portable: Copy `.claude/` folder to new machine, run setup again

---

## 🎯 Command Options

### Full Setup (Default)
```bash
setup:integrations
```
- Installs direnv
- Configures integrations
- Tests connections

### Skip Installation
```bash
setup:integrations --skip-install
```
- Assumes direnv already installed
- Only configures integrations

### Test Only
```bash
setup:integrations --test-only
```
- Only runs integration tests
- Assumes already configured

### Reconfigure
```bash
setup:integrations --reconfigure
```
- Keeps existing values as defaults
- Allows updating specific values

---

## 🔒 Security Features

### 1. Secrets Stay in `.claude/`
- ✅ `.claude/.envrc` is git-ignored
- ✅ Main `.envrc` contains no secrets
- ✅ Safe to commit project `.envrc`

### 2. Token Masking
```markdown
Current Jira API Token: *** (hidden)
Enter new token or press Enter to keep: 
```

### 3. Validation
- ✅ Validates URL formats
- ✅ Validates email formats
- ✅ Tests tokens immediately

### 4. Backup
```bash
# Before reconfiguring
cp .claude/.envrc .claude/.envrc.backup
```

---

## 🎨 Usage Examples

### Example 1: First-time Setup

```bash
User: setup:integrations

AI:
## Prerequisites Check
✅ Shell: zsh
❌ direnv: Not installed

Install direnv? → yes

[Installing direnv...]
✅ direnv installed

## Configure Integrations

### Jira Integration
URL: https://company.atlassian.net
Email: dev@company.com
Token: *** [entered]
Project Key: PROJ
✅ Jira configured

### Confluence Integration
Use Jira credentials? yes
Space Key: TECH
✅ Confluence configured

### Slack Integration
Method: 1 (Bot Token)
Token: *** [entered]
Channel: C1234567890
✅ Slack configured

### Figma Integration
Token: *** [entered]
✅ Figma configured

## Testing Integrations
✅ All 4 integrations working!

✅ Setup Complete!
```

### Example 2: Reconfigure Slack Only

```bash
User: setup:integrations --reconfigure

AI:
## Reconfigure Integrations

Current configuration:
- Jira: ✅ Configured
- Confluence: ✅ Configured
- Slack: ✅ Configured
- Figma: ✅ Configured

Which integration to update?
1. Jira
2. Confluence
3. Slack
4. Figma
5. All

> 3

### Slack Integration
Current: Bot Token (***@company)
1. Keep current
2. Update token
3. Switch to webhook
4. Remove

> 2

New Slack Bot Token: *** [entered]
✅ Slack updated

## Testing Slack
✅ Slack connection working!
```

---

## 🚀 Auto-Continue

After setup completes:
- ✅ Shows summary of configured integrations
- ✅ Shows next steps
- ✅ Offers to start first workflow

```markdown
## Setup Complete! 🎉

**Configured Integrations:**
- ✅ Jira (PROJ)
- ✅ Confluence (TECH space)
- ✅ Slack (#engineering)
- ✅ Figma

**Next Steps:**
1. Try: workflow:start PROJ-1234
2. Try: bugfix "Bug description"
3. Try: document feature "Feature name"

Start your first workflow now? (yes/no)
```

---

## 📋 Troubleshooting

### direnv Not Loading

**Auto-fix:**
```bash
# Check hook installed
grep direnv ~/.zshrc

# Add if missing
echo 'eval "$(direnv hook zsh)"' >> ~/.zshrc

# Reload
source ~/.zshrc

# Allow
direnv allow .
```

### Variables Not Available

**Auto-check:**
```bash
# Test if loaded
echo $JIRA_URL

# If empty, reload
direnv reload

# Or restart shell
exec $SHELL
```

### Integration Test Fails

**Auto-debug:**
```bash
# Show current values (masked)
echo "JIRA_URL: $JIRA_URL"
echo "JIRA_EMAIL: $JIRA_EMAIL"
echo "JIRA_API_TOKEN: ${JIRA_API_TOKEN:0:4}***"

# Test manually
curl -u "$JIRA_EMAIL:$JIRA_API_TOKEN" "$JIRA_URL/rest/api/3/myself"
```

---

## ✅ Success Criteria

**Setup succeeds when:**
- ✅ direnv installed and configured
- ✅ `.claude/.envrc` created with secrets
- ✅ Project `.envrc` loads CCPM config
- ✅ direnv allowed
- ✅ All integrations tested and working
- ✅ Auto-approval updated
- ✅ User can run `workflow:start PROJ-1234`

---

## 🎯 Benefits

**vs Manual Setup:**
- ⚡ 10x faster (2 min vs 20 min)
- ✅ Guided (no docs reading needed)
- ✅ Validated (tests each integration)
- ✅ Secure (proper file structure)
- ✅ Portable (easy to replicate)

**Security:**
- ✅ Secrets in `.claude/` only
- ✅ Main project clean
- ✅ Safe to commit `.envrc`
- ✅ Team-friendly

---

## 📝 Template Used

- `.claude/.envrc.template` - Internal template
- Project `.envrc` - Minimal loader

---

**🎊 One command setup for all integrations! 🚀✨**

