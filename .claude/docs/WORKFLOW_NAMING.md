...
```
.claude/
├── logs/                           # All logs and execution data
...
```bash
bash .claude/scripts/workflow/migrate-workflows.sh
```

**What it does:**
1. ✅ Creates new `logs/workflows/` and `logs/contexts/` structure
2. ✅ Moves deliverables → `logs/contexts/[workflow-id]/deliverables/`
3. ✅ Moves logs → `logs/workflows/[workflow-id]/`
...
cat .claude/workflow-state.json | jq '.workflow_id, .status'
...
`.claude/workflow-state.json` - Active workflow state
...
# Method 3: Script
bash .claude/scripts/workflow/init-workflow.sh "Refactor UserProfile component"
```
...
# Or read state file
cat .claude/workflow-state.json | jq '.workflow_id, .status, .current_phase_name'
```
...
Edit `.claude/scripts/workflow/init-workflow.sh`:
