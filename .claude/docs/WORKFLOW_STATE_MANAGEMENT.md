...
**Location:**
```
.claude/logs/workflows/[workflow-id]/workflow-state.json
```
...
**Location:**
```
.claude/logs/contexts/[workflow-id]/
├── README.md
├── HANDOFF_CONTEXT.md
├── deliverables/
...
```bash
bash .claude/scripts/workflow/workflow-manager.sh list
...
```bash
bash .claude/scripts/workflow/workflow-manager.sh switch [workflow-id]
...
```bash
bash .claude/scripts/workflow/workflow-manager.sh load [workflow-id]
...
```bash
bash .claude/scripts/workflow/workflow-manager.sh delete [workflow-id]
...
```bash
bash .claude/scripts/workflow/workflow-manager.sh archive [workflow-id]
...
bash .claude/scripts/workflow/workflow-manager.sh archive [old-workflow-id]
...
# Loads state from logs/workflows/[id]/
...
```bash
cat .claude/active-workflow.txt
...
```typescript
const workflowId = readFile('.claude/active-workflow.txt').trim();
...
```typescript
writeFile('.claude/active-workflow.txt', newWorkflowId);
```
...
# New session
workflow:resume [id]
# State loads from logs/workflows/[id]/
```
...
**If you have `.claude/workflow-state.json`:**

```bash
# Move to new structure
mkdir -p .claude/logs/workflows/legacy-workflow
mv .claude/workflow-state.json \
   .claude/logs/workflows/legacy-workflow/workflow-state.json

# Set as active
echo "legacy-workflow" > .claude/active-workflow.txt
```
...
1. **Active Workflow Pointer**
   - Always check `.claude/active-workflow.txt` first
...
4. **Handoff & Resume**
   - State persists in `logs/workflows/[id]/`
