...
User: workflow:start "Task"
  ↓
Claude reads: .claude/hooks/pre-phase.md
  ↓
Claude implements logic: Load workflow state, check prerequisites, activate agents
  ↓
Claude executes: Phase 1 logic from .claude/docs/phases/PHASE_1_REQUIREMENTS_ANALYSIS.MD
  ↓
Claude reads: .claude/hooks/post-phase.md
  ↓
...
```
.claude/
├── logs/                          # Created when first workflow runs
...
- ✅ Git-ignored (see `.claude/.gitignore`)
