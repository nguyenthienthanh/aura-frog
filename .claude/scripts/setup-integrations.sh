...
echo -e "${BLUE}🚀 CCPM Integration Setup${NC}"
echo "======================================"
echo ""
...
# Phase 5: Setup project .envrc to load .claude/.envrc
echo -e "${BLUE}Phase 5: Setting up project .envrc...${NC}"
echo ""

CCPM_LOADER="
# CCPM Integration (auto-generated)
if [ -f .claude/.envrc ]; then
  source_env .claude/.envrc
fi"

if [ -f "$PROJECT_ENVRC" ]; then
  if ! grep -q "source_env .claude/.envrc" "$PROJECT_ENVRC"; then
    echo "$CCPM_LOADER" >> "$PROJECT_ENVRC"
    echo -e "${GREEN}✅ Updated $PROJECT_ENVRC to load CCPM config${NC}"
  else
    echo -e "${YELLOW}ℹ️  $PROJECT_ENVRC already loads CCPM config${NC}"
  fi
else
...
if command -v direnv &> /dev/null; then
  ...
  echo "To load manually, run: source .claude/.envrc"
fi
...
if [ -f ".claude/scripts/test-integrations.sh" ]; then
  bash .claude/scripts/test-integrations.sh
else
...
echo ""
echo "🎉 You're ready to use CCPM with integrations!"
