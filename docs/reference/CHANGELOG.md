...
- **Three more hooks made importable + tested** (FEAT-007 / issue #5, batch 8): `post-compact` (`validateStateFile` — per-file required-field checks for workflow-state vs compact-handoff), `pre-execute-load-plan-context` (`composeContextLines` — the mission→…→ready display order, count summaries), `session-start-restore-active` (`composeBanner` — feature-over-initiative-over-mission precedence). 29 tests. (`rate-limit-check` is a single `console.log` reminder — genuinely nothing to extract; left as-is.)
