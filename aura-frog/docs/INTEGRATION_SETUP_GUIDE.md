...
export CONFLUENCE_DEFAULT_LABELS="aura-frog,generated,documentation"  # Optional
...
| `channel_not_found` (Slack) | `/invite @Aura Frog Bot` in channel |
...
bash aura-frog/scripts/confluence-publish.sh 'DEV' 'Aura Frog Test Page' '/tmp/test-doc.md'
```
...
curl -X POST "$SLACK_WEBHOOK_URL" \
  ...
  -d '{"text":"Aura Frog integration test ✅"}'

# Or with bot token
curl -X POST "https://slack.com/api/chat.postMessage" \
  -H "Authorization: Bearer $SLACK_BOT_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"channel\":\"$SLACK_CHANNEL_ID\",\"text\":\"Aura Frog test ✅\"}"

# Test with Aura Frog script
bash aura-frog/scripts/slack-notify.sh '#dev-team' 'Hello from Aura Frog! 👋'
```
...
- ✅ Invite bot to channel: `/invite @Aura Frog Bot`
...
**Create `.env`:**
```bash
cp aura-frog/.envrc.template .env
```
...
**No Additional Costs:**
All integrations use existing service APIs. No extra Aura Frog costs.
...
Aura Frog provides **native Bash script integrations** for external services:
