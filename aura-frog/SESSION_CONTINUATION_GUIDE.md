...
- **Change log** (files created/modified/deleted)
- **Recovery info** (backup branch, stash, backup directory)
...
- ✅ Push to remote if possible (extra safety)
...
- ❌ Switch branches before handoff
- ❌ Delete or rebase the branch before resuming
...
| Item | Captured At | Purpose |
|------|-------------|---------|
| Current branch | Handoff | Verify correct branch on resume |
| Base branch | Start | Know where to merge |
| All commits | Each phase | Track work done |
| Uncommitted files | Handoff | Restore if lost |
| Backup branch | Handoff | Recovery fallback |
| Stash | Handoff | Uncommitted changes backup |

### Recovery Scenarios

**Branch Mismatch:**
```bash
# Agent detects and prompts:
⚠️ Expected: feature/user-auth, Current: main

# Options offered:
1. checkout → Switch to correct branch
2. restore → Use backup branch
...
cp .claude/logs/workflows/[id]/backup/src/* src/
```

**Complete Loss:**
- Deliverables preserved in `.claude/logs/workflows/[id]/`
- Can recreate code from specs and test plans
- Partial restart from Phase 5 with existing planning

### Best Practices

1. **Commit before handoff** - Safest approach
2. **Push to remote** - Extra backup
3. **Don't rebase** - Keeps commit hashes valid
4. **Stay on branch** - Until workflow complete
