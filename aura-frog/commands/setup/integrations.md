...
4. ✅ Generate `.envrc` with your inputs
5. ✅ Update `.gitignore` to ignore secrets
6. ✅ Create/update project `.envrc` to load Aura Frog config
...
```bash
# .envrc - Load Aura Frog integrations
# This file can be committed (no secrets)

# Load Aura Frog integration config
if [ -f .envrc ]; then
  source_env .envrc
fi
...
echo "" >> .envrc
echo "# Aura Frog Integration" >> .envrc
...
✅ .envrc created
✅ Project .envrc updated to load Aura Frog config
✅ direnv allowed
...
```
project/
├── .envrc                    # Loads .envrc (can commit)
│   └── "source_env .envrc"
│
└── aura-frog/
    ├── .envrc                # Aura Frog integrations (git-ignored)
...
- ✅ All secrets in: `.envrc` (git-ignored)
- ✅ Portable: Copy `.envrc` to new machine, run setup again
...
- ✅ `.envrc` created with secrets
- ✅ Project `.envrc` loads Aura Frog config
- ✅ direnv allowed
