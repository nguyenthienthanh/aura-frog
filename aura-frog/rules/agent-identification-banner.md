...
### Line 1: Header
- Version indicator: `AURA FROG v1.18.0`
...
**Show memory status in first response of each session:**
...
|--------|---------|---------|
| Memory loaded | `Memory: X items loaded` | Patterns retrieved from Supabase |
| Memory cached | `Memory: X items (cached)` | Using cached patterns |
| No memory | `Memory: none` | Learning disabled or no data |
| Error | `Memory: error` | Failed to load (check config) |

### Using Loaded Memory

When memory is loaded, Claude should:
1. Read `.claude/cache/memory-context.md` at session start
2. Apply learned patterns to current task
3. Avoid repeating past corrections

**Example memory context file:**
```markdown
# Loaded Memory Context

## Learned Patterns
- **agent_routing**: Use backend-nodejs for Express APIs (85% confidence)
- **code_style**: Prefer early returns in this project (90% confidence)
